...
from sqlalchemy import (
    Column, String, Float, Integer, Boolean, DateTime, ForeignKey, Text, Date,
...
class House(Base):
    ...
    month_start_day = Column(Integer, default=1, nullable=False)  # 1..28
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class Category(Base):
    ...
    is_market_style = Column(Boolean, default=False, nullable=False)  # Mercado-like: tem lista de itens
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class Month(Base):
    ...
    carried_balance = Column(Float, default=0.0, nullable=False)
    closed_at = Column(DateTime, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)

    __table_args__ = (UniqueConstraint("house_id", "year", "month_number", name="uq_house_month"),)
...
class Expense(Base):
    ...
    id = Column(String(36), primary_key=True, default=gen_uuid)
    house_id = Column(String(36), ForeignKey("houses.id", ondelete="CASCADE"), nullable=False, index=True)
    month_id = Column(String(36), ForeignKey("months.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    expense_type = Column(String(20), default="collective", nullable=False)
    split_type = Column(String(20), default="equal", nullable=False)
    has_items = Column(Boolean, default=False, nullable=False)
    is_paid = Column(Boolean, default=True, nullable=False)
    is_recurring_instance = Column(Boolean, default=False, nullable=False)
    recurring_source_id = Column(String(36), nullable=True)
...
class RecurringExpense(Base):
    ...
    category_id = Column(String(36), ForeignKey("categories.id"), nullable=True)
    payer_id = Column(String(36), ForeignKey("users.id"), nullable=False)
    frequency = Column(String(10), default="monthly", nullable=False)  # monthly | weekly | yearly
    day_of_month = Column(Integer, default=1, nullable=False)
    expense_type = Column(String(20), default="collective", nullable=False)
    split_type = Column(String(20), default="equal", nullable=False)
    is_active = Column(Boolean, default=True, nullable=False)
    last_generated_month = Column(String(7), nullable=True)  # "2026-01"
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class ContributionPlan(Base):
    ...
    amount = Column(Float, nullable=False)
    is_active = Column(Boolean, default=True, nullable=False)
    last_generated_month = Column(String(7), nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    __table_args__ = (UniqueConstraint("house_id", "user_id", name="uq_plan_house_user"),)
